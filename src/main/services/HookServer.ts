...
/**
 * Preferred fixed port for the hook server. Picking a stable port means stale
 * `settings.local.json` files left behind by prior runs (or crashes) keep
 * working against the new server, instead of pointing at a dead random port
 * and producing PreToolUse/PostToolUse ECONNREFUSED noise on every tool call.
 *
 * Falls back to a kernel-assigned port if this one is already in use.
 */
const PREFERRED_HOOK_PORT = 61823;
...
class HookServerImpl {
  ...
  private bindServer(port: number): Promise<number> {
  ...
  async start(): Promise<number> {
    ...
    try {
      this._port = await this.bindServer(PREFERRED_HOOK_PORT);
    } catch (err) {
      if ((err as NodeJS.ErrnoException)?.code !== 'EADDRINUSE') throw err;
      console.error(
        `[HookServer] Preferred port ${PREFERRED_HOOK_PORT} in use; falling back to a kernel-assigned port`,
      );
      this._port = await this.bindServer(0);
    }

    // Permanent error handler for runtime errors after binding succeeds
