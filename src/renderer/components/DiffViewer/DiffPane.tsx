...
export interface DiffPaneCommentState {
  count: number;
  /** Send a prompt to the active task with the collected comments and call onAfterSend. */
  addToPrompt: () => void;
}
...
export interface DiffPaneProps {
  ...
  /** Forwarded so the parent (and MinimapRail) can drive scroll. */
  scrollContainerRef: React.RefObject<HTMLDivElement>;
  /** Called by the "Add to prompt" flow to dismiss the modal after sending. */
  onClose: () => void;
  /**
   * Reports outward changes to the comment state so the shell can render the
   * "Add N comments to prompt" button in its header. Stable callback expected.
   */
  onCommentStateChange?: (state: DiffPaneCommentState) => void;
  /** Hunks to visually highlight. Wired in Task 7; ignore the value for now. */
  highlightedHunkIndices?: number[];
  /** Click handler on the hunk header. Wired in Task 7; ignore for now. */
...
export function DiffPane({
  ...
  useEffect(() => {
    ...
    function handleMouseUp(e: MouseEvent) {
      setSelection((prev) => {
        ...
        if (row && scrollContainerRef.current) {
          ...
          setPopover({
            top: rowRect.bottom - containerRect.top + scrollContainerRef.current.scrollTop + 4,
    ...
    const containerAtMount = scrollContainerRef.current;
    return () => {
      containerAtMount?.classList.remove('select-none');
      document.removeEventListener('mousemove', handleMouseMove);
      document.removeEventListener('mouseup', handleMouseUp);
    };
  }, [selection?.active, findLineFromPoint, diff, lineIndex, scrollContainerRef]);
  ...
  const handleAddToPrompt = useCallback(() => {
    ...
    onClose();
  }, [activeTaskId, comments, diff, onClose]);

  // Bubble comment state up to the shell so it can render the header button.
  useEffect(() => {
    if (!onCommentStateChange) return;
    onCommentStateChange({ count: comments.length, addToPrompt: handleAddToPrompt });
  }, [comments.length, handleAddToPrompt, onCommentStateChange]);
