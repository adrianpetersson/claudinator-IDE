...
export class TerminalSessionManager {
  ...
  constructor(opts: {
    ...
    // Register file path link provider — clicks open the diff in focus mode
    // (the inline diff panel) via a custom event that App.tsx subscribes to.
        ...
        (filePath) => {
          window.dispatchEvent(
            new CustomEvent('claudinator:open-file', {
              detail: { taskId: sessionId, filePath },
            }),
          );
        },
      ),
